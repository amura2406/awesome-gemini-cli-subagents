...
GLOBAL_AGENTS_DIR="$HOME/.gemini/agents"
LOCAL_AGENTS_DIR=".gemini/agents"
GEMINI_AGENTS_DIR=""  # Will be set by select_install_mode
INSTALL_MODE=""  # "global" or "local"
SOURCE_MODE=""  # "local" or "remote"

# GitHub API configuration
GITHUB_API_BASE="https://api.github.com/repos/ankitmundada/awesome-gemini-cli-subagents/contents"
GITHUB_RAW_BASE="https://raw.githubusercontent.com/ankitmundada/awesome-gemini-cli-subagents/main"

# Cache for remote data
REMOTE_CATEGORIES=()
REMOTE_AGENTS=()

# Function to check if local .gemini directory exists
has_local_gemini_dir() {
    [[ -d ".gemini" ]]
}
...
select_install_mode() {
    ...
    echo -e "  ${YELLOW}1)${NC} Global installation ${CYAN}(~/.gemini/agents/)${NC}"
    echo -e "     Available for all projects"
    echo ""

    if has_local_gemini_dir; then
        echo -e "  ${YELLOW}2)${NC} Local installation ${CYAN}(.gemini/agents/)${NC}"
        echo -e "     Only for current project"
    else
        echo -e "  ${BLUE}2)${NC} Local installation ${CYAN}(not available)${NC}"
        echo -e "     ${YELLOW}No .gemini/ directory found in current directory${NC}"
    fi
    echo ""
    echo -e "  ${YELLOW}q)${NC} Quit"
    echo ""
    ...
    case "$choice" in
        1)
            GEMINI_AGENTS_DIR="$GLOBAL_AGENTS_DIR"
            INSTALL_MODE="global"
            mkdir -p "$GEMINI_AGENTS_DIR"
            ;;
        2)
            if has_local_gemini_dir; then
                GEMINI_AGENTS_DIR="$LOCAL_AGENTS_DIR"
                INSTALL_MODE="local"
                mkdir -p "$GEMINI_AGENTS_DIR"
            else
                echo -e "\n${RED}Local installation not available. No .gemini/ directory found.${NC}"
                sleep 2
                select_install_mode
                return
            fi
...
show_header() {
    ...
    echo -e "${BOLD}${CYAN}"
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║           Gemini CLI Agents Installer                       ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    if [[ -n "$INSTALL_MODE" ]]; then
        local mode_str=""
        if [[ "$INSTALL_MODE" == "global" ]]; then
            mode_str="Global (~/.gemini/agents/)"
        else
            mode_str="Local (.gemini/agents/)"
        fi
...
is_agent_installed() {
    ...
    [[ -f "$GEMINI_AGENTS_DIR/$agent_name" ]]
}
...
select_agents() {
    ...
    if [[ "$SOURCE_MODE" == "remote" ]]; then
        ...
        for agent_file in "${REMOTE_AGENTS[@]}"; do
            ...
            if [[ -f "$GEMINI_AGENTS_DIR/$agent_file" ]]; then
                agent_states+=(1)
            else
                agent_states+=(0)
            fi
        ...
        for agent_file in "$category_path"/*.md; do
            ...
            if [[ "$basename" != "README.md" ]]; then
                agents+=("$basename")
                if [[ -f "$GEMINI_AGENTS_DIR/$basename" ]]; then
                    agent_states+=(1)
                else
                    agent_states+=(0)
                fi
...
confirm_and_apply() {
    ...
    if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
        ...
        for agent_file in "${to_install[@]}"; do
            if [[ -n "$agent_file" ]]; then
                if [[ "$SOURCE_MODE" == "remote" ]]; then
                    ...
                    if download_agent "$category" "$agent_file" "$GEMINI_AGENTS_DIR/$agent_file"; then
                    ...
                    if [[ -f "$source_path" ]]; then
                        cp "$source_path" "$GEMINI_AGENTS_DIR/$agent_file"
                        echo -e "${GREEN}✓${NC} Installed: $agent_file"
                    fi
                fi
            fi
        ...
        for agent_file in "${to_uninstall[@]}"; do
            if [[ -n "$agent_file" ]]; then
                if [[ -f "$GEMINI_AGENTS_DIR/$agent_file" ]]; then
                    rm "$GEMINI_AGENTS_DIR/$agent_file"
